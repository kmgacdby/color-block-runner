...
public class UtilitySuiteClient implements ClientModInitializer {
 ...
 private static void tick(MinecraftClient client){long w=client.getWindow().getHandle();UnifiedConfig c=UnifiedConfig.get();boolean menu=key(w,GLFW.GLFW_KEY_O);if(menu&&!menuWasDown&&client.currentScreen==null)client.setScreen(new UtilitySuiteScreen(null));menuWasDown=menu;
