...
public class UtilitySuiteScreen extends Screen {
    ...
    @Override public void render(DrawContext d, int mx, int my, float delta) {
        ...
        d.drawTextWithShadow(textRenderer, "Menu key: " + keyName(UnifiedConfig.get().menuKey), left, bindY, 0xFFD9D0D9);
        d.drawTextWithShadow(textRenderer, "Click the key label below to change it", left, bindY + 18, 0xFF938A94);
    }

    private void drawDetails(DrawContext d, int x, int y, int mx, int my) {
        UnifiedConfig c = UnifiedConfig.get();
        if (selected == 0) {
            toggleRow(d, x, y, "Enabled", c.stewEnabled, mx, my);
            slider(d, x, y + 38, "Move delay", c.stewDelay, 0, 20, mx, my, 1, " ticks");
            d.drawTextWithShadow(textRenderer, "Delay between each transfer", x, y + 70, 0xFFAAA1AB);
        } else if (selected == 1) {
            toggleRow(d, x, y, "Enabled", c.runnerEnabled, mx, my);
            slider(d, x, y + 38, "Search range", c.runnerRange, 4, 64, mx, my, 2, " blocks");
            toggleRow(d, x, y + 76, "Auto sprint", c.runnerSprint, mx, my);
            toggleRow(d, x, y + 114, "Auto jump", c.runnerJump, mx, my);
        } else {
            toggleRow(d, x, y, "Enabled", c.dodgeEnabled, mx, my);
            slider(d, x, y + 38, "Detection range", c.dodgeRange, 4, 64, mx, my, 2, " blocks");
            d.drawTextWithShadow(textRenderer, "Dodge priority", x, y + 76, 0xFFEDE7ED);
            d.drawTextWithShadow(textRenderer, "LEFT/RIGHT > BACK/FORWARD > DIAGONAL", x + 120, y + 76, 0xFFFFB8E5);
            toggleRow(d, x, y + 114, "Ignore own arrows", c.ignoreOwnArrows, mx, my);
            toggleRow(d, x, y + 152, "Show trajectory", c.showLanding, mx, my);
            d.drawTextWithShadow(textRenderer, "Jump and sneak are currently not used by dodge", x, y + 184, 0xFFAAA1AB);
        }
    }

    private void toggleRow(DrawContext d, int x, int y, String label, boolean value, int mx, int my) {
    ...
    private void toggleDetail(UnifiedConfig c, int id) {
        if (selected == 1) {
            if (id == 3) c.runnerSprint = !c.runnerSprint;
            else if (id == 4) c.runnerJump = !c.runnerJump;
        } else if (selected == 2) {
            if (id == 5) c.ignoreOwnArrows = !c.ignoreOwnArrows;
            else if (id == 6) c.showLanding = !c.showLanding;
        }
        c.save();
    }

    private void setSlider(UnifiedConfig c, int id, double mouseX) {
        int x = 295, sw = width - 30 - x;
        double t = Math.max(0, Math.min(1, (mouseX - x) / (double) sw));
        if (id == 1) c.stewDelay = (int) Math.round(t * 20);
        else if (id == 2) {
            int min = 4, max = 64;
            if (selected == 1) c.runnerRange = min + (int) Math.round(t * (max - min));
            else c.dodgeRange = min + (int) Math.round(t * (max - min));
        }
        c.save();
    }
    ...
    @Override public boolean mouseClicked(double mx, double my, int button) {
        ...
        if (mx >= left && mx <= left + w) {
            int i = (int) ((my - top) / row);
            if (i >= 0 && i < 3) {
                if (button == 0) { toggle(i); selected = i; }
                else if (button == 1) selected = i;
                else if (button == 2) { selected = i; startBinding(i); }
                return true;
            }
        }
        ...
        if (mx >= rx && mx <= width - 30) {
            UnifiedConfig c = UnifiedConfig.get();
            if (selected == 0) {
                if (my >= 97 && my <= 125) { setSlider(c, 1, mx); dragging = 1; return true; }
            } else if (selected == 1) {
                ...
                if (my >= 135 && my <= 170) { setSlider(c, 2, mx); dragging = 2; return true; }
                if (my >= 209 && my <= 241) { toggleDetail(c, 5); return true; }
                if (my >= 247 && my <= 279) { toggleDetail(c, 6); return true; }
            }
        }
    ...
    @Override public boolean keyPressed(int key, int scan, int mods) {
        if (binding >= 0) {
            if (key == GLFW.GLFW_KEY_ESCAPE) { binding = -1; return true; }
            UnifiedConfig c = UnifiedConfig.get();
            if (binding == 0) c.stewKey = key; else if (binding == 1) c.runnerKey = key; else c.dodgeKey = key;
            c.save(); binding = -1; return true;
        }
