...
public final class ArrowDodge {
    ...
    private static List<Vec3d> simulatePath(MinecraftClient client,Vec3d start,Vec3d velocity){
        List<Vec3d> points=new ArrayList<>();Vec3d p=start,v=velocity;points.add(p);double maxSq=UnifiedConfig.get().dodgeRange*(double)UnifiedConfig.get().dodgeRange;
        for(int i=0;i<MAX_SIM_TICKS;i++){Vec3d next=p.add(v);if(next.squaredDistanceTo(client.player.getPos())>maxSq&&p.squaredDistanceTo(client.player.getPos())>maxSq)break;BlockHitResult hit=client.world.raycast(new RaycastContext(p,next,RaycastContext.ShapeType.COLLIDER,RaycastContext.FluidHandling.NONE,client.player));if(hit.getType()!=HitResult.Type.MISS){points.add(hit.getPos());break;}points.add(next);p=next;v=v.multiply(DRAG).subtract(0,GRAVITY,0);if(p.y<client.world.getBottomY()-2)break;}return points;
    }
    private static boolean willHitPlayer(MinecraftClient client,List<Vec3d> path){Box box=client.player.getBoundingBox().expand(0.12);for(int i=1;i<path.size();i++)if(box.raycast(path.get(i-1),path.get(i)).isPresent())return true;return false;}
    ...
    private static boolean clearsPath(MinecraftClient client,List<Vec3d> path,Vec3d offset){Box box=client.player.getBoundingBox().offset(offset).expand(.12);for(int i=1;i<path.size();i++)if(box.raycast(path.get(i-1),path.get(i)).isPresent())return false;return !client.world.getBlockCollisions(client.player,box).iterator().hasNext();}
    ...
    public static void render(WorldRenderContext context){MinecraftClient client=MinecraftClient.getInstance();UnifiedConfig c=UnifiedConfig.get();if(!c.dodgeEnabled||!c.showLanding||trajectory.size()<2||client.player==null)return;VertexConsumerProvider consumers=context.consumers();if(consumers==null)return;VertexConsumer buffer=consumers.getBuffer(RenderLayer.getLines());Matrix4f matrix=context.matrixStack().peek().getPositionMatrix();Vec3d camera=context.camera().getPos();for(int i=1;i<trajectory.size();i++){Vec3d a=trajectory.get(i-1).subtract(camera),b=trajectory.get(i).subtract(camera);buffer.vertex(matrix,(float)a.x,(float)a.y,(float)a.z).color(255,90,90,220).next();buffer.vertex(matrix,(float)b.x,(float)b.y,(float)b.z).color(255,180,90,220).next();}}
}
