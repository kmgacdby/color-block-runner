...
public class ColorBlockRunnerClient implements ClientModInitializer {
    ...
    public static void tickExternal(MinecraftClient client) {
        ...
        Config.enabled = u.runnerEnabled;
        Config.range = u.runnerRange;
        Config.autoJump = u.runnerJump;
        Config.autoSprint = u.runnerSprint;
    ...
    private static void findTarget(MinecraftClient client) {
        ...
        for (int x = cx0 - r; x <= cx0 + r; x++) for (int z = cz0 - r; z <= cz0 + r; z++) {
            ...
            if (ColorBlock.getDyeColor(client.world.getBlockState(new BlockPos(x, y, z)).getBlock()) == heldColor) {
                best = d;
    ...
    private static boolean isAtTarget(MinecraftClient client) {
        ...
        // Treat the center of a small target as reached before the player can overshoot it.
        double dx = client.player.getX() - (target.getX() + 0.5);
        double dz = client.player.getZ() - (target.getZ() + 0.5);
        return Math.abs(dx) <= 0.34 && Math.abs(dz) <= 0.34;
    }
    ...
    private static void moveTowardTarget(MinecraftClient client) {
        ...
        client.options.jumpKey.setPressed(distance > 1.15 && (Config.autoJump || savedJump));
        client.options.sprintKey.setPressed(distance > 1.15 && (Config.autoSprint || savedSprint));
        if (Config.autoSprint && distance > 1.15) client.player.setSprinting(true);
        controlling = true;
    }
